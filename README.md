...
⚠️ Development version — Beta

This repository contains the actively developed version of LeHaMoC. Although the core numerical framework has been validated and published, new features included here may still be under testing and validation. The code interface and numerical implementation may therefore evolve between releases.
...
5. **g_min_el**: Minimum Lorentz factor of electrons on the grid (log_10).

6. **g_max_el**: Maximum Lorentz factor of electrons on the grid (log_10).

7. **g_el_PL_min**: Minimum Lorentz factor of power-law electrons (log_10).

8. **g_el_br**: Break Lorentz factor of the injected electron distribution. If `g_el_br = 0`, the code uses a single power law from "g_el_PL_min" to "g_el_PL_max". If `g_el_br != 0`, the code uses a broken power law with break Lorentz factor `gamma_br_el = 10**g_el_br`  (log_10).

9. **g_el_PL_max**: Maximum Lorentz factor of power-law electrons (log_10).

10. **grid_g_el**: Number of grid points between g_min_el and g_max_el.

11. **g_min_pr**: Minimum Lorentz factor of protons on the grid (log_10).

12. **g_max_pr**: Maximum Lorentz factor of protons on the grid (log_10).

13. **g_pr_PL_min**: Minimum Lorentz factor of power-law protons (log_10).

14. **g_pr_br**: Break Lorentz factor of the injected particle distribution for protons. If `g_pr_br = 0`, the code uses a single power law from "g_pr_PL_min" to "g_pr_PL_max". If `g__pr_br != 0`, the code uses a broken power law with break Lorentz factor `gamma_br_pr = 10**g_pr_br` (log_10).

15. **g_pr_PL_max**: Maximum Lorentz factor of power-law protons (log_10).
...
20. **L_el**: Luminosity of electrons in erg s^{-1} (log_10).
...
23. **L_pr**: Luminosity of protons in erg s^{-1} (log_10).
